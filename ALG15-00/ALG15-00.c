...
*/
int atom_search (node *n, int x, int nivel_atual) {
    if (n == NULL) return -1; // node nao inicializado
    if (n->type == ATOM) {
        if (n->head.atom == x) {
            return nivel_atual;
        }
    } else { // n-type é LIST
        //incrementa o nivel quando entra no conteudo da SUBLISTA
        int resultado_vertical = atom_search(n->head.list, x, nivel_atual+1);
        //se for direfente de -1 retorna o resultado que encontrou na primeira ocorrencia
        if (resultado_vertical != -1) {
            return resultado_vertical;
        }
    }

    return atom_search(n->tail, x, nivel_atual);
}
int profundidade(node *n) {
	...
	if (n->type == LIST) {
		...
		int profundidade_interna;
		profundidade_interna = profundidade(n->head.list);

		int profundidade_maxima_interna = 1 + profundidade_interna;

		return (profundidade_maxima_interna > profundidade_cauda) ? profundidade_maxima_interna : profundidade_cauda;
	}
	return 0;
}
...
int main() {
	...
	int nivel_procurado = -1; // inicializa uma variavel para regular se achou ou nao o atomo

    if (root != NULL && root->head.list != NULL) 
    {
        nivel_procurado = atom_search(root->head.list, atomo_procurado, 0);
    }
